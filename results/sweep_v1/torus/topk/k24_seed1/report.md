# Stage 1 Sign-off — `torus` / `topk` (m=128, seed=1)
...
- [x] Correct region is contiguous (n_correct_total = 1)
- [x] Correct region is interior to grid (no edge contact)
- [x] Failure modes outside correct region are interpretable (b₁ monotone non-decreasing in n_intervals at each fixed overlap)
- [ ] Spectral log-ratio error E < 0.05 on post-activations (E=0.1299)
