"""Build the sweep_helix manifest: 33 per-run YAML configs + manifest.tsv.
...
JumpReLU is intentionally excluded (scrapped per analysis decision).

Run dir layout: results/sweep_v1/helix/<arch>/<knob_str>_seed<s>/  (unified
with sweep_v1 so the same aggregate.py invocation discovers helix alongside
circle/torus/sphere).
Config layout:  configs/sweep_helix/<run_id>.yaml
Manifest:       configs/sweep_helix/manifest.tsv
...
SLURM_ARRAY_TASK_ID. This is why helix gets its own configs namespace
(separate manifest, separate sbatch).
"""
...
CONFIGS_DIR = REPO / "configs" / "sweep_helix"
RESULTS_DIR = REPO / "results" / "sweep_v1"      # unified namespace
LOGS_DIR = REPO / "logs" / "sweep_helix"
...
# Explicit helix kwargs match Helix class defaults and configs/stage1_smoke_helix.yaml.
# Keeping them explicit prevents silent drift if Helix.__init__ defaults change later.
FIXED_DGP = {
    "d": 64,
    "sigma": 0.01,
    "radius": 1.0,
...
def _build_cell(arch: str, value, seed: int) -> dict:
    ...
    return {
        ...
        "topology": TOPOLOGY,
        "arch": arch,
        "sparsity_knob": {"relu_l1": "l1_coeff", "topk": "k"}[arch],
...
def _enumerate_cells():
    ...
    for value in L1_GRID:
        for seed in SEEDS:
            cells.append(_build_cell("relu_l1", value, seed))
    for value in K_GRID:
        for seed in SEEDS:
            cells.append(_build_cell("topk", value, seed))
    return cells
...
def main() -> None:
    ...
    print(f"  relu_l1: {n_relu}   topk: {n_topk}")
